...
class DhanConfig:
    ...
    access_token: str = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiJ9.eyJpc3MiOiJkaGFuIiwicGFydG5lcklkIjoiIiwiZXhwIjoxNzY2MjY1MDcyLCJhcHBfaWQiOiJjOTNkM2UwOSIsImlhdCI6MTc2NjE3ODY3MiwidG9rZW5Db25zdW1lclR5cGUiOiJBUFAiLCJ3ZWJob29rVXJsIjoiIiwiZGhhbkNsaWVudElkIjoiMTEwMDQ4MDM1NCJ9.D2bXDHxAoVn0dTtVDjpzjIb5Xn7oYWxrbVfBEeCaAIDyfW4bfjPQODtP_CUghD8VfDR1slb4pRst_RJeMPyZoQ"

# Popular NSE Stock Options with Security IDs  
DHAN_STOCK_IDS = {
    # Banking & Financial
    "HDFCBANK": 1333,
    "ICICIBANK": 4963,
...
STOCK_CONFIG = {
    # Banking & Financial
    "HDFCBANK": {"lot_size": 550, "strike_interval": 20},
    "ICICIBANK": {"lot_size": 1375, "strike_interval": 10},
    "SBIN": {"lot_size": 1500, "strike_interval": 5},
    "KOTAKBANK": {"lot_size": 400, "strike_interval": 25},
    "AXISBANK": {"lot_size": 1200, "strike_interval": 10},
    
    # IT
    "TCS": {"lot_size": 150, "strike_interval": 50},
    "INFY": {"lot_size": 300, "strike_interval": 25},
    "WIPRO": {"lot_size": 1200, "strike_interval": 5},
    "HCLTECH": {"lot_size": 350, "strike_interval": 25},
    "TECHM": {"lot_size": 580, "strike_interval": 25},
    
    # Energy & Power
    "RELIANCE": {"lot_size": 250, "strike_interval": 25},
    "ONGC": {"lot_size": 2850, "strike_interval": 5},
    "POWERGRID": {"lot_size": 3900, "strike_interval": 5},
    "NTPC": {"lot_size": 3000, "strike_interval": 5},
    
    # Auto
    "TATAMOTORS": {"lot_size": 1350, "strike_interval": 10},
    "M&M": {"lot_size": 300, "strike_interval": 25},
    "BAJAJ-AUTO": {"lot_size": 125, "strike_interval": 100},
    
    # Pharma
    "SUNPHARMA": {"lot_size": 400, "strike_interval": 25},
    "DRREDDY": {"lot_size": 125, "strike_interval": 50},
    
    # FMCG
    "ITC": {"lot_size": 1600, "strike_interval": 5},
    "HINDUNILVR": {"lot_size": 300, "strike_interval": 50},
    "BRITANNIA": {"lot_size": 50, "strike_interval": 100},
    
    # Metals
    "TATASTEEL": {"lot_size": 5750, "strike_interval": 5},
    "HINDALCO": {"lot_size": 1175, "strike_interval": 10},
    
    # Telecom
    "BHARTIARTL": {"lot_size": 425, "strike_interval": 25},
}
...
class DhanStockOptionsFetcher:
    ...
    def fetch_rolling_data(self, stock: str, from_date: str, to_date: str, 
                          strike_type: str = "ATM", option_type: str = "CALL", 
                          interval: str = "60", expiry_code: int = 1):
        try:
            security_id = DHAN_STOCK_IDS.get(stock, 1333)
    ...
    def process_historical_data(self, stock: str, target_date: str, strikes: List[str], 
                               interval: str = "60", expiry_code: int = 1):
        ...
        config = STOCK_CONFIG.get(stock, STOCK_CONFIG["RELIANCE"])
        ...
        for strike_type in strikes:
            ...
            for i, ts in enumerate(timestamps):
                try:
                    ...
                    call_gex = (call_oi * call_gamma * spot_price**2 * lot_size) / 1e9
                    put_gex = -(put_oi * put_gamma * spot_price**2 * lot_size) / 1e9
                    call_dex = (call_oi * call_delta * spot_price * lot_size) / 1e9
                    put_dex = (put_oi * put_delta * spot_price * lot_size) / 1e9
                    
                    call_vanna_exp = (call_oi * call_vanna * spot_price * lot_size) / 1e9
                    put_vanna_exp = (put_oi * put_vanna * spot_price * lot_size) / 1e9
                    call_charm_exp = (call_oi * call_charm * spot_price * lot_size) / 1e9
                    put_charm_exp = (put_oi * put_charm * spot_price * lot_size) / 1e9
...
def main():
    ...
    <div class="main-header">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <div>
                ...
                <p class="sub-title">Historical Stock Options Greeks Analysis | Dhan Rolling API | Indian Standard Time</p>
            </div>
    ...
    with st.sidebar:
        ...
        # Stock categories
        stock_categories = {
            "Banking & Financial": ["HDFCBANK", "ICICIBANK", "SBIN", "KOTAKBANK", "AXISBANK"],
            "IT": ["TCS", "INFY", "WIPRO", "HCLTECH", "TECHM"],
            "Energy & Power": ["RELIANCE", "ONGC", "POWERGRID", "NTPC"],
            "Auto": ["TATAMOTORS", "M&M", "BAJAJ-AUTO"],
        ...
        st.info(f"""
        **{stock}**  
        Lot Size: {stock_info.get('lot_size', 'N/A')}  
        Strike Interval: ₹{stock_info.get('strike_interval', 'N/A')}
        """)
        ...
        st.info("📊 Stock options are monthly expiry contracts")
    ...
    if fetch_button or (hasattr(st.session_state, 'fetch_config') and st.session_state.get('data_fetched', False)):
        ...
        if not st.session_state.get('data_fetched', False) or 'df_data' not in st.session_state:
            ...
            <div class="metric-card neutral" style="margin: 20px 0;">
                ...
                <div class="metric-value" style="color: #3b82f6; font-size: 1.2rem;">
                    {stock} | {target_date} | {interval} min | Strikes: {', '.join(strikes[:3])}...
                </div>
                <div class="metric-delta">Monthly expiry contract | This may take 1-3 minutes...</div>
            </div>
            """, unsafe_allow_html=True)
            
            try:
                fetcher = DhanStockOptionsFetcher(DhanConfig())
                df, meta = fetcher.process_historical_data(stock, target_date, strikes, interval, expiry_code)
        ...
        st.success(f"✅ Data fetched successfully! Total records: {len(df):,} | Strikes: {meta['strikes_count']} | Lot Size: {meta['lot_size']}")
        ...
        st.info(f"""
        📊 **Calculation Method**: Market metrics below are calculated using only the **nearest 6 strikes (±3 from ATM)** around stock price ₹{spot_price:,.2f}. 
        This focuses on strikes with actual market impact. Uses **{stock}** lot size ({config['lot_size']} shares per contract). All selected strikes are displayed in charts for comprehensive analysis.
        """)
        ...
        st.info("""
        👋 **Welcome to NYZTrade Stock Options GEX/DEX Dashboard!**
        
        **Available Stocks (25+ F&O Stocks):**
        ...
        **Key Features:**
        - 📊 **NET GEX/DEX** analysis with stock-specific lot sizes
        - 🎯 **Extended Strikes** - Up to ATM ±10 for comprehensive view
        - 🌊 **Flow Analysis** - Track institutional positioning
        - ⏰ **VANNA & CHARM** - Advanced Greeks for volatility analysis
        - 📈 **Intraday Timeline** - Complete evolution charts
        - 📋 **12 Analysis Tabs** - Professional options analytics
        
        **Calculation Methodology:**
        - **Market Metrics** (GEX Suppression/Amplification, DEX Direction) calculated using **nearest 6 strikes (±3 from ATM)**
        - **Stock-specific lot sizes** for accurate exposure calculation
        - **Monthly expiry contracts** for longer-term positioning
        - All selected strikes displayed in charts for comprehensive analysis
        
        **How to use:**
        1. Select stock category and stock
        2. Choose date and monthly expiry
        3. Select strikes (up to ±10)
        4. Click "Fetch Stock Options Data"
        5. Navigate through 12 comprehensive tabs
        
        **💡 Pro Tip:** Liquid stocks like RELIANCE, HDFCBANK, TCS have better data quality. Check lot sizes as they vary significantly per stock.
        """)
    ...
    st.markdown(f"""<div style="text-align: center; padding: 20px; color: #64748b;">
        ...
        NYZTrade Stock Options GEX/DEX Dashboard | Data: Dhan Rolling API | IST<br>
        25+ NSE F&O Stocks | Monthly Expiry | 12 Analysis Tabs | Stock-Specific Lot Sizes<br>
        Extended Strikes (±10) | NET GEX/DEX Analysis | Professional Stock Options Analytics</p>
    </div>""", unsafe_allow_html=True)
